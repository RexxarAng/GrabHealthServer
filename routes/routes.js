...
router.post('/forgetpassword', (req, res) => {
    ...
    if(!Validator.validateNric(req.body.nric)) {
        return res.status(404).json({success: false, msg: "Invalid nric."})
    }
    ...
    currentRole.getUserByEmail(req.body.email, (err, user) => {
        ...
        if(user) {
            if (user.nric === req.body.nric) {
                    ...
                    if(error) {
                        ...
                        bcrypt.genSalt(10, (err, salt) => {
                            bcrypt.hash(randomPassword, salt, (err, hash) =>{
                                ...
                                return res.json({success: true, msg: "Your password has successfully been reset. Please check your email."});
                            });
                        });
                ...
                return res.json({success: false, msg: "Either the email or nric is incorrect"});
            }
        } else {
            return res.json({success: false, msg: "Either the email or nric is incorrect"});
        }
